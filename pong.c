...
void init_pong(struct Pong* pong, struct Sounds* sounds) {
    ...
    init_paddle(&pong->player2, (TABLE_WIDTH - PADDLE_WIDTH - PADDLE_X_OFFSET), 
        (TABLE_HEIGHT - PADDLE_HEIGHT - PADDLE_Y_OFFSET), PADDLE_WIDTH, PADDLE_HEIGHT);
    
    init_ball(&pong->ball, (TABLE_WIDTH / 2 - BALL_SIZE / 2), (TABLE_HEIGHT / 2 - BALL_SIZE / 2), BALL_SIZE);
    
    init_middle_line(&pong->middle_line, (TABLE_WIDTH / 2 - MID_LINE_WIDTH / 2), 0, 
        MID_LINE_WIDTH, TABLE_HEIGHT);
...
void update_pong(struct Pong* pong, double dt) {
    if (pong->state == PLAY)
    {
        ...
        else if (collides(ball_hitbox, player2_hitbox))
        {
            ...
            /*When there is a collision between the ball and player2, he paddle stops*/
            pong->player2.vy = 0; 
        ...
        /*If the ball has passed half the court and its vx is positive*/
        if(ball_hitbox.x1 >= pong->middle_line.x && pong->ball.vx > 0) {
            /*We stop the paddle if the ball is between its coordinates y1 and y2*/
            if(ball_hitbox.y1 > player2_hitbox.y1 && ball_hitbox.y2 < player2_hitbox.y2) {
                pong->player2.vy = 0;
            }
            /*We raise the paddle if the ball is a above it*/
            else if(ball_hitbox.y1 < player2_hitbox.y1) {
                pong->player2.vy = -PADDLE_SPEED;
            } 
            /*We lover the paddle if the ball is below it*/
...
void render_pong(struct Pong pong, struct Fonts fonts) {
    render_middle_line(pong.middle_line);
    ...
    char player_message[7];
    sprintf(player_message, "%s", "Player");
    al_draw_text(fonts.large_font, al_map_rgb(255, 255, 255), TABLE_WIDTH / 2 - 50, TABLE_HEIGHT / 3, ALLEGRO_ALIGN_CENTER, player_message);
    sprintf(player_message, "%s", "CPU");
    al_draw_text(fonts.large_font, al_map_rgb(255, 255, 255), TABLE_WIDTH / 2 + 50, TABLE_HEIGHT / 3, ALLEGRO_ALIGN_CENTER, player_message);
    ...
    else if (pong.state == DONE)
    {
        ...
        /*Condition to print which player won the game*/
        char winner_message[12];
        if(pong.winning_player == 1) {
            sprintf(winner_message, "Player won!");
            al_draw_text(fonts.large_font, al_map_rgb(255, 255, 255), TABLE_WIDTH / 2, TABLE_HEIGHT / 1.5, ALLEGRO_ALIGN_CENTER, winner_message);
        } else {
